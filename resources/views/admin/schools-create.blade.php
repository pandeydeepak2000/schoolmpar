...
@section('title', 'Add New School – SchoolMapr Admin')
...
<div style="max-width:1080px;" x-data="{
    ...
    previews: {
        main: 'https://images.unsplash.com/photo-1580582932707-520aed937b7b?auto=format&fit=crop&w=800&q=80',
        classroom: 'https://images.unsplash.com/photo-1580582932707-520aed937b7b?auto=format&fit=crop&w=800&q=80',
        activity: 'https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&w=800&q=80',
        laboratory: 'https://images.unsplash.com/photo-1513258496099-48168024aec0?auto=format&fit=crop&w=800&q=80',
        facilities: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=800&q=80',
        campus: 'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=800&q=80'
    },
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
        <div>
            <h2 style="font-size:22px; font-weight:800; color:#0f2d59; margin:0;">Add New School to SchoolMapr</h2>
            <p style="font-size:13px; color:#64748b; margin:4px 0 0;">Create a verified school profile in Patna District or across Bihar</p>
        </div>
    ...
    <form action="{{ route('admin.schools.crud.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div style="background:#fff; border:1.5px solid #e8ecf4; border-radius:14px; padding:24px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
            <h3 style="font-size:15px; font-weight:800; color:#0f2d59; margin:0 0 18px; padding-bottom:12px; border-bottom:1.5px solid #f1f5f9;">
                🏫 School Details & Location (Patna / Bihar)
            </h3>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                <div style="grid-column:1/-1;">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required
                           placeholder="e.g. St. Anne's High School Patna"
                ...
                <div>
                    ...
                    <input type="text" name="city" value="{{ old('city', 'Kankarbagh') }}" required
                ...
                <div>
                    ...
                    <input type="text" name="address" value="{{ old('address') }}" required
                           placeholder="e.g. Housing Board Colony, Near Tempo Stand, Kankarbagh, Patna - 800020"
                           style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                </div>
            </div>
            ...
            <div style="margin-top:16px;">
                ...
                <textarea name="description" rows="3" placeholder="Brief overview of the school's history, ethos, achievements..."
                          style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; resize:vertical; box-sizing:border-box;">{{ old('description') }}</textarea>
            </div>
        </div>

        {{-- Panel 2: 5-SLOT PHOTO GALLERY MANAGER --}}
        <div style="background:#fff; border:1.5px solid #e8ecf4; border-radius:14px; padding:24px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; padding-bottom:12px; border-bottom:1.5px solid #f1f5f9;">
                <div>
                    ...
                    <p style="font-size:12px; color:#64748b; margin:2px 0 0;">
                        Upload photos or enter direct image URLs. These appear on the public school details gallery.
                    </p>
                </div>
        ...
        <div style="background:#fff; border:1.5px solid #e8ecf4; border-radius:14px; padding:24px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:16px;">
                <div>
                    <label style="font-size:12px; font-weight:700; color:#374151; display:block; margin-bottom:5px;">Upload Prospectus File (PDF / DOC / Image):</label>
                ...
                <div>
                    ...
                    <input type="url" name="prospectus_url" value="{{ old('prospectus_url') }}"
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:16px;">
                ...
                <div>
                    ...
                    <select name="school_type" style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                        <option value="Co-Ed" selected>Co-Educational</option>
                ...
                <div>
                    ...
                    <input type="number" name="class_from" value="{{ old('class_from', 1) }}" min="1" max="12" required
                           style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                </div>

                <div>
                    <label style="font-size:12px; font-weight:700; color:#374151; display:block; margin-bottom:5px;">Class To *</label>
                    <input type="number" name="class_to" value="{{ old('class_to', 12) }}" min="1" max="12" required
                           style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                </div>
        ...
        <div style="background:#fff; border:1.5px solid #e8ecf4; border-radius:14px; padding:24px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:16px; margin-bottom:18px;">
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" placeholder="e.g. +91 612 2345678"
                           style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                </div>
                <div>
                    <label style="font-size:12px; font-weight:700; color:#374151; display:block; margin-bottom:5px;">Official Email</label>
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="e.g. principal@school.com"
                           style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                </div>
                <div>
                    ...
                    <select name="admission_status" required style="width:100%; border:1.5px solid #cbd5e1; border-radius:8px; padding:10px 14px; font-size:14px; outline:none; box-sizing:border-box;">
                        <option value="open" selected>🟢 Open 2026-27</option>
